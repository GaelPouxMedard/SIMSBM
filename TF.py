...
def parse_args():
	...
	parser.add_argument('--fname', type = str, default = '/', help = 'Model filename (modif Gael)')
	
	parser.add_argument('--verbose', type = int, default = 1, help = 'Verbose or not')
	return parser.parse_args()

def TPTF(X, Xtest, dims, rank, reg, reg_S, lr, lrS, max_iter, tol=1e-4):
	'''
	...
	while iter_count < max_iter:
		...
		for i in np.random.permutation(ninstance): # pick on instance in X randomly
			...
			for k in range(ndims):
				...
				incr = lr * e_ijk * tensor_ttv(core, Ui_list, k)
				step += incr
				U[k][ind[k]] -= step
				Ui_list = [U[k][ind[k]] for k in range(ndims)]
			...
			#print(np.sum(np.isnan(Ui_list)), np.sum(np.isnan(core)), np.shape(core))
			core -= step_S
...
def tensor_ttv(core, Ui_list, remove_k=None):
	...
	if remove_k is not None:
		modes=list(modes).remove(remove_k)
	#
